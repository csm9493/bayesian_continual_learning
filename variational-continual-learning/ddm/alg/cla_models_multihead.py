...
def weight_variable(shape, init_weights=None):
    if init_weights is not None:
        ...
        init_range = math.sqrt(1 / din)
        Wi_val = tf.random_uniform([din, dout], -init_range, init_range)
...
#             np.random.shuffle(perm_inds)
            perm_inds = np.random.permutation(x_train.shape[0])
            ...
            for i in range(total_batch):
                ...
                _, c = sess.run(
                    ...
                    feed_dict={self.x: batch_x, self.y: batch_y, self.task_idx: task_idx}, options=run_options)
    ...
    def prediction(self, x_test, task_idx):
        # Test model
        prediction = self.sess.run([self.pred], feed_dict={self.x: x_test, self.task_idx: task_idx},options=run_options)[0]
        return prediction

    def prediction_prob(self, x_test, task_idx):
        prob = self.sess.run([tf.nn.softmax(self.pred)], feed_dict={self.x: x_test, self.task_idx: task_idx}, options=run_options)[0]
        return prob

    def get_weights(self):
        weights = self.sess.run([self.weights], options=run_options)[0]
        return weights
...
#                 bi_val = tf.truncated_normal([dout], stddev=0.1)
                init_range = math.sqrt(1 / din)
                Wi_val = tf.random_uniform([din, dout], -init_range, init_range)
                bi_val = tf.random_uniform([dout], -init_range, init_range)
            else:
...
class MFVI_NN(Cla_NN):
    def __init__(self, input_size, hidden_size, output_size, training_size, 
        no_train_samples=2, no_pred_samples=2, prev_means=None, prev_log_variances=None, learning_rate=0.001, 
...
#                 bi_m_val = tf.truncated_normal([dout], stddev=0.1)
                ...
                init_range = math.sqrt(1 / din)
                Wi_m_val = tf.random_uniform([din, dout], -init_range, init_range)
                bi_m_val = tf.random_uniform([dout], -init_range, init_range)
