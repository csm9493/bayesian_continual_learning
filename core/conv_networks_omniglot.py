...
class BayesianConvNetwork(nn.Module):
    def __init__(self, inputsize, taskcla, init_type = 'random', rho_init = -4.6001):
        ...
        self.conv1 = BayesianConv2D(ncha,64,kernel_size=3, rho_init = rho_init)
        s = compute_conv_output_size(size,3) # 26
        self.conv2 = BayesianConv2D(64,64,kernel_size=3, rho_init = rho_init)
        s = compute_conv_output_size(s,3) # 24
        s = s//2 #12
        self.conv3 = BayesianConv2D(64,64,kernel_size=3, rho_init = rho_init)
