...
def train_cl(model, train_datasets, test_datasets, replay_mode="none", scenario="class",classes_per_task=None,iters=2000,batch_size=32,
             ...
             use_exemplars=True, add_exemplars=False, eval_cbs_exemplars=list(), train_info = None):
    ...
    all_acc_for_save = np.zeros((num_task, num_task), dtype=np.float32)
...
#         iters_left = iters_left_previous = 1
        ...
        for epoch in range(1, iters_to_use+1):
            ...
            data_loader = utils.get_data_loader(train_dataset, batch_size, cuda=cuda, drop_last=True)
            data_loader_previous = data_loader
            if Exact:
                if scenario=="task":
                    up_to_task = task if replay_mode=="offline" else task-1
            ...
            # -----------------Collect data------------------#
            for (batch_idx, data), (_, data_previous) in zip(enumerate(data_loader), enumerate(data_loader_previous)): 
            #####-----CURRENT BATCH-----#####
                if replay_mode=="offline" and scenario=="task":
                    x = y = scores = None
                else:
                    x, y = data             #--> sample training data of current task
                ...
                loss_total_dict['loss_total'] += loss_dict['loss_total']
                loss_total_dict['loss_current'] += loss_dict['loss_current']
                loss_total_dict['loss_replay'] += loss_dict['loss_replay']
                loss_total_dict['pred'] += loss_dict['pred']
                loss_total_dict['pred_r'] += loss_dict['pred_r']
                loss_total_dict['distil_r'] += loss_dict['distil_r']
                loss_total_dict['ewc'] += loss_dict['ewc']
                loss_total_dict['si_loss'] += loss_dict['si_loss']
                loss_total_dict['precision'] += loss_dict['precision']
            ...
            loss_total_dict['loss_total'] /= (batch_idx+1)
            loss_total_dict['loss_current'] /= (batch_idx+1)
            loss_total_dict['loss_replay'] /= (batch_idx+1)
            loss_total_dict['pred'] /= (batch_idx+1)
            loss_total_dict['pred_r'] /= (batch_idx+1)
            loss_total_dict['distil_r'] /= (batch_idx+1)
            loss_total_dict['ewc'] /= (batch_idx+1)
            loss_total_dict['si_loss'] /= (batch_idx+1)
            loss_total_dict['precision'] /= (batch_idx+1)
            
            # Fire callbacks (for visualization of training-progress / evaluating performance after each task)
            for loss_cb in loss_cbs:
                if loss_cb is not None:
                    loss_cb(progress, epoch, loss_total_dict, task=task)
            ...
            if generator is not None and batch_index <= gen_iters:
                ...
                for loss_cb in gen_loss_cbs:
                    if loss_cb is not None:
                        loss_cb(progress_gen, epoch, loss_total_dict, task=task)
                for sample_cb in sample_cbs:
                    if sample_cb is not None:
        ...
        for u in range(task):
            ...
            acc = evaluate.validate(model, test_datasets[u], verbose=False, test_size=None, task=u+1, with_exemplars=False, 
                                    allowed_classes=list(range(classes_per_task*u, classes_per_task*(u+1))) if scenario=="task" else None)
            print('>>> Test on task {:2d} : acc={:5.1f}% <<<'.format(u, 100 * acc))
            all_acc_for_save[task-1, u] = acc
                             
        
        # Save

        log_name = '{}_{}_{}epochs_{}_{}units_lamb{}_{}'.format(train_info['date'], train_info['experiment'], iters, train_info['alg'], train_info['units'], train_info['reg_coef'], train_info['trial'])
