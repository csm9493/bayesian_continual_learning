...
def get(seed=0,fixed_order=True,pc_valid=0.15, tasknum=10):
    data={}
    taskcla=[]
    size = {}
        ...
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                for i,folder in enumerate(folders):
                    ...
                    for img_idx, ims in enumerate(img_list):
                        s = 'train'
                        if img_idx >= 500:
                            s = 'test'
                ...
                s_arr = ['train','test']
                directory_arr = ['/train', '/val']
                
                for s, directory, folders in zip(s_arr, directory_arr, folders_arr):
                    for i, folder in enumerate(folders):
                        folder_path = os.path.join(root+directory, folder)
                        img_list = sorted(os.listdir(folder_path))
                        if(i==200):
                            print(folder)
...
#                 std=(0.2752,)
                ...
                dat={}
                dat['train']=TrafficSigns('../dat/traffic_signs', train=True, download=True, transform=transforms.Compose([
                    transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=TrafficSigns('../dat/traffic_signs', train=False, download=True, transform=transforms.Compose([
                    transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
            ...
            # "Unify" and save
            for s in ['train','test']:
                data[n][s]['x']=torch.stack(data[n][s]['x']).view(-1,size[n][0],size[n][1],size[n][2])
